...
random.seed(39)
np.random.seed(39)
# To disable default ssl verification inorder to circumvent the error - SSL: CERTIFICATE_VERIFY_FAILED
...
def read_prepro_write_dataset(dataset, split, output_directory):
    ...
    Inputs:
      ...
      output_directory: directory to write the preprocessed files
    Returns:
    ...
    for articles_id in tqdm(range(len(dataset['data'])), desc="Preprocessing {} dataset".format(split)):
    ...
    with open(os.path.join(output_directory, split + '.context'), 'w') as context_file,  \
            open(os.path.join(output_directory, split + '.question'), 'w') as question_file,\
            open(os.path.join(output_directory, split + '.answer'), 'w') as ans_text_file, \
            open(os.path.join(output_directory, split + '.span'), 'w') as span_file:
